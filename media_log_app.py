...
PAGE_SIZE = 100   # entries per page in Browse
...
PLATFORM_LOGOS = {
    ...
    "JioHotstar":      "",   # REPLACE: data:image/png;base64,<b64>
    "SonyLiv":         "",
    "ZEE5":            "",
    "Disney+ Hotstar": "",
    "Other":           "",
...
def tmdb_search(title: str, media_type: str) -> dict:
    """Return {year, genres, poster_url} or empty dict on failure."""
    ...
    try:
        ...
        # Genre map (partial – covers common ids)
        gmap = {
        ...
        genres = list(dict.fromkeys(gmap.get(gid, "Other") for gid in genre_ids[:3]))
        poster = ""
        if top.get("poster_path"):
            poster = TMDB_IMG_BASE + top["poster_path"]
        return {"year": year, "genres": genres, "poster": poster}
    except Exception:
        return {}
...
def platform_badge(platform: str) -> str:
    ...
    if logo:
        return (
            f'<img src="{logo}" width="16" height="16" '
...
def status_badge(status: str) -> str:
    ...
    return (
        f'<span style="background:{color};color:#ffffff;padding:2px 9px;'
...
def recommend_badge(recommend: str) -> str:
    ...
    if r == "yes":
        return '<span style="background:#16a34a;color:#ffffff;padding:2px 9px;border-radius:999px;font-size:0.72rem;font-weight:500;">👍 Yes</span>'
    if r == "no":
        return '<span style="background:#6b7280;color:#ffffff;padding:2px 9px;border-radius:999px;font-size:0.72rem;font-weight:500;">👎 No</span>'
    return ""
...
<div style="font-size:0.75rem;margin-top:4px;">
  ...
  <div style="display:flex;height:5px;border-radius:999px;overflow:hidden;margin-top:3px;background:#374151;">
...
def page_add_entry(entries_ws):
    ...
    # TMDB autofill section (outside form so it can trigger reactively)
    with st.expander("🔍 Auto-fill from TMDB (optional)", expanded=False):
        af_col1, af_col2, af_col3 = st.columns([3, 1, 1])
        with af_col1:
            tmdb_title_input = st.text_input("Search title on TMDB",
                                             placeholder="e.g. Inception",
                                             key="tmdb_title_input")
        with af_col2:
            tmdb_type_input = st.selectbox("Type", ["movie", "series"], key="tmdb_type_input")
        with af_col3:
            st.write("")
            st.write("")
            search_tmdb = st.button("Search", key="tmdb_search_btn")

        if search_tmdb and tmdb_title_input.strip():
            result = tmdb_search(tmdb_title_input.strip(), tmdb_type_input)
            if result:
                st.session_state["tmdb_result"] = result
                st.session_state["tmdb_query"]  = tmdb_title_input.strip()
                st.session_state["tmdb_type"]   = tmdb_type_input
                cols = st.columns([1, 3])
                with cols[0]:
                    if result.get("poster"):
                        st.image(result["poster"], width=80)
                with cols[1]:
                    st.success(
                        f"Found: **{tmdb_title_input.strip()}** "
                        f"({result.get('year','?')})  \n"
                        f"Genres: {', '.join(result.get('genres', []))}"
                    )
                    st.caption("Click **Use this data** below to pre-fill the form.")
                if st.button("✅ Use this data", key="tmdb_use_btn"):
                    st.session_state["prefill_title"]   = tmdb_title_input.strip()
                    st.session_state["prefill_year"]    = result.get("year", "")
                    st.session_state["prefill_genres"]  = result.get("genres", [])
                    st.session_state["prefill_type"]    = tmdb_type_input
                    st.rerun()
            else:
                st.warning("No results found. Try a different title.")

    # Pre-fill defaults from TMDB (if used)
    pf_title  = st.session_state.pop("prefill_title",  "")
    pf_year   = st.session_state.pop("prefill_year",   "")
    pf_genres = st.session_state.pop("prefill_genres", [])
    pf_type   = st.session_state.pop("prefill_type",   "movie")
    ...
    with st.form("add_entry_form", clear_on_submit=True):
        ...
        with c2:
            title = st.text_input(
                ...
                help="Use the original title if possible. Check if this entry is already added before submitting.",
            )

        c3, c4, c5 = st.columns(3)
        with c3:
            type_options = ["movie", "series"]
            type_idx = type_options.index(pf_type) if pf_type in type_options else 0
            media_type = st.selectbox("Type", type_options, index=type_idx)
        with c4:
        ...
        c6, c7 = st.columns(2)
        with c6:
            # Multiselect for genres — pre-fill from TMDB if available
            valid_pf = [g for g in pf_genres if g in GENRES_LIST]
            genre_sel = st.multiselect(
                "Genre",
                options=GENRES_LIST,
                default=valid_pf if valid_pf else [],
                help="Select one or more genres that fit best.",
            )
        with c7:
            language = st.selectbox("Language", LANGUAGES, index=0)

        # Rating / Recommend / Year — hide entirely for "plan"
        rating    = None
        recommend = ""
        ...
        if status != "plan":
            ...
            with c10:
                year_val = int(pf_year) if pf_year and pf_year.isdigit() else datetime.now().year
    ...
    if submitted:
        ...
        if errors:
            ...
            genre_str = ", ".join(genre_sel) if genre_sel else ""
            row = {
...
def page_browse(entries_ws, votes_ws):
    ...
    watched = df[df["status"] == "watched"]       if "status" in df.columns else df
    rec_pct = (
        int(100 * (watched["recommend"] == "yes").sum() / max(len(watched), 1))
    ...
    if all(c in df.columns for c in ["recommend", "status", "rating"]):
        ...
        if not top_pool.empty:
            ...
            st.caption("Random picks from your top-rated, recommended entries.")
            picks = top_pool.sample(min(3, len(top_pool)))
    ...
    # ── Voter name (sidebar keeps it persistent) ──
    ...
    # ── My entries filter ─────────────────────
    my_name = st.session_state.get("sidebar_name", "").strip()
    ...
    with st.expander("Filters", expanded=False):
        ...
        with fc1:
            plat_opts = sorted(
                df["platform"].dropna().replace("", "Unknown").unique().tolist()
            )
        ...
        with fc3:
            stat_f = st.multiselect(
                "Status",
                sorted(df["status"].dropna().unique().tolist()) if "status" in df.columns else [],
                key="f_stat",
            )
        with fc4:
            rec_f = st.multiselect(
                "Rec",
                sorted(df["recommend"].dropna().unique().tolist()) if "recommend" in df.columns else [],
                key="f_rec",
            )
        with fc5:
            # Explode comma-separated genres for filter options
            all_genres = sorted(set(
                g.strip()
                for genres_str in df.get("genre", pd.Series(dtype=str)).dropna()
                for g in str(genres_str).split(",")
                if g.strip()
            ))
            genre_f = st.multiselect("Genre", all_genres, key="f_genre")

    # ── Apply preset first, then manual filters ──
    filtered = df.copy()

    if preset == "Recommended only":
        filtered = filtered[filtered.get("recommend", pd.Series(dtype=str)) == "yes"]
    ...
    if show_mine and my_name:
        filtered = filtered[
            filtered["added_by"].str.strip().str.lower() == my_name.lower()
        ]
    ...
    voter_input = st.text_input(
        "Your name (used for voting)",
        value=voter_name,
        placeholder="Enter your name to vote on entries",
        key="voter_name_input",
    )
    if voter_input.strip():
        st.session_state["voter_name"] = voter_input.strip()

    st.caption("Enter your name above once, then 👍/👎 any entry below.")

    # ── Export + View toggle ──────────────────
    exp_col, view_col = st.columns([1, 2])
    with exp_col:
        st.download_button(
            "⬇ Export CSV",
    ...
    page_data  = filtered.iloc[page_start:page_end]
    ...
    if total_pages > 1:
        ...
        with pg_c2:
            st.markdown(
                ...
                f"Page {st.session_state.browse_page} of {total_pages} "
                f"({total_filtered} entries)</div>",
                unsafe_allow_html=True,
            )
        with pg_c3:
...
def page_reports(entries_ws):
    ...
    tab1, tab2, tab3 = st.tabs(["By Platform", "By Genre", "By Added By"])
    ...
    with tab3:
        if "added_by" in df.columns:
            ...
            st.bar_chart(ac)
            avg_by_person = df.groupby("added_by")["rating"].mean().round(1)
            avg_by_person.name = "Avg Rating"
            st.dataframe(avg_by_person.reset_index(), use_container_width=True)
...
<div class="wlog-card">
  <div style="display:flex;justify-content:space-between;align-items:flex-start;">
    ...
    <div style="display:flex;align-items:center;gap:6px;">{platform_html}</div>
  </div>
  ...
  {review_html}
  <div style="margin-top:7px;">{comm_bar}</div>
...
def _render_vote_widget(entry_id, title_txt, voter_name,
    ...
    col_label, col_yes, col_no, col_spacer = st.columns([3, 1, 1, 6])

    with col_label:
        if not voter_name:
            st.markdown(
                '<span style="font-size:0.75rem;color:#9ca3af;">Enter your name above to vote</span>',
                unsafe_allow_html=True,
            )
            ...
            st.markdown(
                '<span style="font-size:0.75rem;color:#9ca3af;">Community vote:</span>',
                unsafe_allow_html=True,
            )
    ...
    if voter_name and not voted_in_sheet and not voted_in_session:
        ...
        with col_no:
            if st.button("👎", key=f"no_{entry_id}_{card_idx}",
                         help=f"Wouldn't recommend — {title_txt}"):
                try:
...
# ─────────────────────────────────────────────
#  TABLE RENDERER (no comments column)
# ─────────────────────────────────────────────
def _render_table(filtered, vote_summary):
    ...
    # Comments excluded from table — visible in Cards only
    ...
    # Title-case Type and Status display values in the raw column (already done via badge for status)
...
def main():
    ...
    # My entries name — persists across Browse
    my_name_input = st.sidebar.text_input(
        "Your name",
        value=st.session_state.get("sidebar_name", ""),
        placeholder="e.g. Pankaj",
        help="Used to pre-fill forms and enable 'My entries' filter.",
        key="sidebar_name_input",
    )
    if my_name_input.strip():
        st.session_state["sidebar_name"]  = my_name_input.strip()
        st.session_state["saved_name"]    = my_name_input.strip()
        st.session_state["voter_name"]    = my_name_input.strip()
